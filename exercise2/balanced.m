function n = balanced(img_sequence, tx, ty)
%PLAY Summary of this function goes here
%   Detailed explanation goes here

[h, w, c, n] = size(img_sequence)

figure(3), imshow(img_sequence(:,:,:,1))
imwrite(img_sequence(:,:,:,1), strcat('stabilised/', sprintf('%08d', 1), '.jpg'))
orgX = tx(1);
orgY = ty(1);
...
for i = 2:n
    ...
    x1 = max(1, 1 - dX);
    x2 = min(w, w - dX);                                                    %prej je blo w - dX
    y1 = max(1, 1 - dY);
    y2 = min(h, h - dY);                                                    %prej je blo h - dy
    ...
    size(transposed)
    imshow(transposed)
    imwrite(transposed, strcat('stabilised/', sprintf('%08d', i), '.jpg'))
    %waitforbuttonpress
end
